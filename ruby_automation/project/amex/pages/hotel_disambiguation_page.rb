...
class HotelDisambiguationPage < BasePage    
  def initialize(driver, test_env)   
    @db_object = Page.find_by_name('hotel disambiguation page')
    @url = Environment.find_by_name(test_env).base_url + Page.find_by_name('hotel disambiguation page').url
    @title = Page.find_by_name('hotel disambiguation page').title
    @driver = driver 
  end    
	...
	def search_hotels_button  
	  wait = Selenium::WebDriver::Wait.new(:timeout => TIMEOUT)
	  wait.until {
	  @search_hotels_button = self.driver.find_element(:css => self.db_object.elements.find_by_name('search hotels button').by_css_locator)
